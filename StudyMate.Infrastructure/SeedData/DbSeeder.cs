...
public class DbSeeder
{
    ...
    public async Task Seed()
    {
        ...
        if (!instructorResult.Succeeded)
            throw new Exception("Ошибка при создании instructor: " + string.Join(", ", instructorResult.Errors.Select(e => e.Description)));

        var studentResult = await _userManager.CreateAsync(student, "Test@123");
        if (!studentResult.Succeeded)
            throw new Exception("Ошибка при создании student: " + string.Join(", ", studentResult.Errors.Select(e => e.Description)));
        ...
        if (instructor == null || student == null)
            throw new Exception("Instructor или student не найдены после создания");
